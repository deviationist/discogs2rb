...
def main():
    ...
    tracks = get_tracks_with_missing_album()
    if not tracks:
    ...
    logger.info(f"[cyan]Writing result to CSV-file...")
    write_csv(resolved_tracks)

def write_csv(resolved_tracks):
    header_columns = ["TrackID", "TrackTitle", "TrackArtist", "AlbumTitle", "AlbumArtists", "ReleaseYear", "ReleaseDate", "ReleaseURL", "Artwork"]
    data = [header_columns] + resolved_tracks
    try:
        with open("output.csv", mode="w", newline="") as file:
            writer = csv.writer(file)
            writer.writerows(data)
            logger.info(f"[cyan][bold green]✔ Done!")
